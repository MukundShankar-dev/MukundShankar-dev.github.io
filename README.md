...
The narrative explains the movements, learning setup, failures, and decisions;
internal checkpoint labels are not reader-facing headings. Replay image provenance is recorded in
...
2. Extend the narrative near the end of the active phase, using descriptive
   headings rather than internal checkpoint numbers. Keep calendar dates and
   date ranges out of phase headings; work on the project is intermittent.
   ...
   when they explain the decision. Introduce the inputs, movement commands,
   controller, and learning objective before reporting metrics. Distinguish
   observed failures from suspected causes, scripted control from learned
   policies, and development checks from untouched held-out evaluation.
   Cut repeated caveats and audit bookkeeping, not the explanation of how things
   work. Do not invent failures or personal reactions.
3. Preserve earlier results. Explain a later correction in its chronological
   ...
   Progress logs sometimes revisit earlier checkpoints; use the actual sequence
   of experiments, not the ordering implied by internal checkpoint IDs.
...
5. Keep the homepage focused on the project goal: a high-level orchestrator
   requests reusable skills instead of generating low-level movement commands.
   This is a design goal, not a measured token-cost reduction. Put completed experiments,
...
the homepage and journal checks, and `npm run build` for the production build. The existing
